...
const API_BASE = (web_url || "").trim().replace(/\/+$/, "");
...
function index() {
  ...
  const handleKNN = (imgId) => {
    setLoading(true);
    fetch(`${API_BASE}/imgsearch?imgid=${imgId}&k=${k}`, fetchGetObj)
      .then((res) => res.json())
      .then((data) => {
        handleData(data);
        setLoading(false);
  ...
  const toggleFullScreen = (image) => {
    if (image !== null) {
      fetch(`${API_BASE}/relatedimg?imgid=${image.id}`, fetchGetObj)
        .then((res) => res.json())
        .then((data) => {
